...
void Game::lancer(){
    ...
    std::cout << "pas ressorti" << std::endl;
    ...
    this->terrain.print();
        ...
        int y = pos[1];
        if (dir == Dir::DROITE){
            if (terrain.get_value({x+1,y})== '#' || terrain.get_value({x+1,y})== '+'){
                joueur.move(Dir::DROITE);
                dir = Dir::DROITE;
            }
            else if (terrain.get_value({x,y-1})== '#' || terrain.get_value({x,y-1})== '+'){
                joueur.move(Dir::HAUT);
                dir = Dir::HAUT;
            }
            else if (terrain.get_value({x,y+1})== '#' || terrain.get_value({x,y+1})== '+'){
            ...
            if (terrain.get_value({x,y-1})== '#' || terrain.get_value({x,y-1})== '+'){
                joueur.move(Dir::HAUT);
                dir = Dir::HAUT;
            }
            else if (terrain.get_value({x+1,y})== '#' || terrain.get_value({x+1,y})== '+'){
                joueur.move(Dir::DROITE);
                dir = Dir::DROITE;
            }
            else if (terrain.get_value({x-1,y})== '#' || terrain.get_value({x-1,y})== '+'){
